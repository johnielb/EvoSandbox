...
def build_primitives(bound1, bound2):
...
def build_toolbox(x_train, y_train, classifier):
    bound1, bound2 = x_train[0, :, :].shape
    pset = build_primitives(bound1, bound2)
    ...
    def evalTrain(individual):
        ...
        clf = classifier()
        accuracy = round(100 * cross_val_score(clf, train_norm, y_train, cv=3).mean(), 2)
        return accuracy,
...
toolbox = build_toolbox(x_train, y_train, LinearSVC)
...
if __name__ == "__main__":
    ...
    pop, log, hof = train(randomSeeds, toolbox)
